...
def mid_dct_transform(Ex_dct1_out, Ex_dct2_in,
    ...
    for i in range(index, N, stride):
        Ex_bet[i, 0] = Ey_bet[i, 0] = By_bet[i, 0] = 0
...
@numba.cuda.jit
def unpack_transverse_fields(Ex_dct2_out, Ey_dct2_out, By_dct2_out,
                             Ex, Ey, By):
    N = Ex.shape[0]
    index = numba.cuda.grid(1)
    stride = numba.cuda.blockDim.x * numba.cuda.gridDim.x
...
class GPUMonolith:
    ...
    def __init__(self, config):
        ...
        self.dct_plan = pyculib.fft.FFTPlan(shape=(2 * N - 2,),
                                            ...
                                            batch=(3 * N))
        # (2 * N - 2) // 2 + 1 == (N - 1) + 1 == N
        self._combined_dct1_in = numba.cuda.device_array((3 * N, 2 * N - 2))
        self._combined_dct1_out = numba.cuda.device_array((3 * N, N), dtype=np.complex128)
        self._combined_dct2_in = numba.cuda.device_array((3 * N, 2 * N - 2))
        self._combined_dct2_out = numba.cuda.device_array((3 * N, N), dtype=np.complex128)
        ...
        self._By_dct1_in = self._combined_dct1_in[2*N:3*N, :]
        self._By_dct1_out = self._combined_dct1_out[2*N:3*N, :]
        self._By_dct2_in = self._combined_dct2_in[2*N:3*N, :]
        self._By_dct2_out = self._combined_dct2_out[2*N:3*N, :]
    ...
    def load(self, plasma, beam_ro, Ex_prev, Ey_prev, By_prev, jx_prev, jy_prev):
        Nc = self._Nc
    ...
    def initial_deposition(self, config, plasma_initial):
        self.load(plasma_initial, 0, 0, 0, 0, 0, 0)
    ...
    def calculate_Ex_Ey_By(self):
        # The grand plan: mul * iDCT(SPECTRAL_MAGIC(DCT(in.T).T)).T).T
        # where iDCT is DCT;
        # and DCT is jury-rigged from symmetrically-padded DFT
        self.calculate_Ex_Ey_By_1()
        self.calculate_Ex_Ey_By_2()
        self.calculate_Ex_Ey_By_3()
        self.calculate_Ex_Ey_By_4()

    def calculate_Ex_Ey_By_1(self):
    ...
    def calculate_Ex_Ey_By_2(self):
        ...
        mid_dct_transform[self.cfg](self._Ex_dct1_out, self._Ex_dct2_in,
                                    ...
                                    self._Ex_bet, self._Ey_bet, self._By_bet,
                                    self._mix_alf, self.mix_mul)
        numba.cuda.synchronize()

    def calculate_Ex_Ey_By_3(self):
    ...
    def calculate_Ex_Ey_By_4(self):
        # 4. Transpose the resulting Ex (TODO: fuse this step into later steps?)
        unpack_transverse_fields[self.cfg](self._Ex_dct2_out,
                                           self._Ey_dct2_out,
                                           self._By_dct2_out,
                                           self._Ex, self._Ey, self._By)
        numba.cuda.synchronize()


    def step(self, config, plasma, beam_ro, Ex_prev, Ey_prev, By_prev, jx_prev, jy_prev):
        self.load(plasma, beam_ro, Ex_prev, Ey_prev, By_prev, jx_prev, jy_prev)

        self.deposit()
        self.calculate_RHS()

        self.calculate_Ex_Ey_By()
