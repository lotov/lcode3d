...
@numba.cuda.jit
def calculate_RHS_kernel(Ex_sub, By_sub,
                         beam_ro, ro, jx, jx_prev, jz,
                         grid_step_size, xi_step_size, subtraction_trick,
                         Ex_dct1_in, By_dct1_in):
    N = ro.shape[0]
    index = numba.cuda.grid(1)
    stride = numba.cuda.blockDim.x * numba.cuda.gridDim.x
    for k in range(index, ro.size, stride):
...
@numba.cuda.jit
def mid_dct_transform(Ex_dct1_out, Ex_dct2_in, By_dct1_out, By_dct2_in,
                      Ex_bet, By_bet, alf, mul):
    ...
    for i in range(index, N, stride):
        ...
        Ex_dct2_in[N - 1, i] = By_dct2_in[N - 1, i] = 0  # Note the forced zero
...
@numba.cuda.jit
def unpack_transverse_fields(Ex_dct2_out, By_dct2_out, Ex, By):
    N = Ex.shape[0]
    index = numba.cuda.grid(1)
    stride = numba.cuda.blockDim.x * numba.cuda.gridDim.x
...
class GPUMonolith:
    ...
    def load(self, plasma, beam_ro, Ex_prev, By_prev, jx_prev):
        Nc = self._Nc
    ...
    def initial_deposition(self, config, plasma_initial):
        self.load(plasma_initial, 0, 0, 0, 0)
    ...
    def calculate_Ex_By(self):
        # The grand plan: mul * iDCT(SPECTRAL_MAGIC(DCT(in.T).T)).T).T
        # where iDCT is DCT;
        # and DCT is jury-rigged from symmetrically-padded DFT
        self.calculate_Ex_By_1()
        self.calculate_Ex_By_2()
        self.calculate_Ex_By_3()
        self.calculate_Ex_By_4()
    ...
    def calculate_Ex_By_4(self):
        ...
        unpack_transverse_fields[self.cfg](self._Ex_dct2_out, self._By_dct2_out,
                                           self._Ex, self._By)
        numba.cuda.synchronize()


    def step(self, config, plasma, beam_ro, Ex_prev, By_prev, jx_prev):
        self.load(plasma, beam_ro, Ex_prev, By_prev, jx_prev)

        self.deposit()
        self.calculate_RHS()

        self.calculate_Ex_By()
